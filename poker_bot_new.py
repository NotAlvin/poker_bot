...
TOKEN = "5820327452:AAG3Tn1lD2DMvV-YuW-UdMjk_qv_XsYycOQ"
...
def transfer(update: Update, context: CallbackContext) -> None:
    ...
    update.message.reply_text('Please select the person you want to transfer chips to:', reply_markup=reply_markup)
...
def handle_transfer_selection(update: Update, context: CallbackContext) -> None:
    ...
    from_user_data = players_data[from_user_id]
    from_user_data.setdefault("transfers", [])

    transfer_data = {"type": "send", "amount": 0, "to": to_user_name, "to_id": to_user_id}
    from_user_data["transfers"].append(transfer_data)
...
def handle_transfer_amount(update: Update, context: CallbackContext) -> None:
    ...
    # Find the transfer with an empty "amount" field in the user's transfers list
    transfer_to = next((t for t in players_data[from_user_id]["transfers"] if t["type"] == "send" and t["amount"] == 0), None)
    if not transfer_to:
        get_message(update).reply_text('Please use /transfer command to initiate a transfer.')
        return ConversationHandler.END

    to_user_id = transfer_to["to_id"]
    to_user_name = transfer_to["to"]
    ...
    to_user_data = players_data[to_user_id]
    to_user_data.setdefault("transfers", [])
    to_user_data["transfers"].append({"type": "receive", "amount": amount, "from": from_user_name})

    get_message(update).reply_text(f"{from_user_name} transferred ${amount} to {to_user_name}")
    return ConversationHandler.END

    from_user_id = update.effective_user.id
    from_user_name = get_user_name(update.effective_user)

    # Find the transfer with an empty "amount" field in the user's transfers list
    transfer_to = next((t for t in players_data[from_user_id]["transfers"] if t["type"] == "send" and not t["amount"]), None)
    if not transfer_to:
        update.message.reply_text('Please use /transfer command to initiate a transfer.')
        return ConversationHandler.END

    to_user_id = transfer_to["recipient_id"]
    to_user_name = transfer_to["recipient_name"]
    ...
    # Update recipient's transfers list
    players_data[to_user_id]["transfers"].append({"type": "receive", "amount": amount, "from": from_user_name})

    update.message.reply_text(f"{from_user_name} transferred ${amount:.2f} to {to_user_name}")
    return ConversationHandler.END
